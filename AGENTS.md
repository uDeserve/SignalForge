...
Do not commit real secrets from `.env`. Update `.env.example` when config requirements change, and keep `feedbackmesh.agent.json`, `feedbackmesh.integration.json`, `docs/quick-start.md`, and `docs/deployment.md` aligned whenever the install flow changes. Hosted examples currently point at `https://feedbackmesh.launchhub.icu`; treat that as deployment state, not a naming source of truth.
